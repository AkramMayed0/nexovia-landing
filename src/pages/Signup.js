...
const PLANS = {
  starter:    { name: 'Starter',    price: '$0/mo',  color: '#4f8eff' },
  pro:        { name: 'Pro',        price: '$29/mo', color: '#7c3aed' },
  enterprise: { name: 'Enterprise', price: '$99/mo', color: '#10b981' },
};
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.free) {
        setSuccess(true); // Show success message for free plan
      } else if (data.url) {
        window.location.href = data.url; // Redirect to Stripe for paid plans
      }
    } catch (e) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative' }}>
      ...
      <div style={{ width: '100%', maxWidth: 460, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 20, padding: '36px 32px' }}>
          ...
          <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 99, background: planInfo.color + '18', border: '1px solid ' + planInfo.color + '33', fontSize: 12, fontWeight: 700, color: planInfo.color, marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--text2)', marginBottom: 28 }}>
            {plan === 'starter' ? "Start for free — no credit card required." : "Complete your details then proceed to payment."}
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading} className="glow-btn" style={{ width: '100%', padding: '13px', borderRadius: 12, fontSize: 15, marginTop: 8, opacity: loading ? 0.7 : 1 }}>
              {loading ? 'Setting up…' : plan === 'starter' ? 'Create Free Workspace →' : 'Continue to Payment →'}
            </button>
          </form>
